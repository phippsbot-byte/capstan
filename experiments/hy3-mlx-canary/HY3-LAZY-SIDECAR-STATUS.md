...
Hybrid q4 runtime cut: artifact `cpp/results/20260708-hybrid-q4-summary.json`. C++ routed-MoE replay and the persistent daemon now expose `dense|direct|hybrid` q4 modes (`--q4-mode`, plus Python `HY3_CPP_ROUTE_Q4_MODE`). Default remains `dense`; `hybrid` uses direct q4 for cold/low-reuse experts and dense+Accelerate when an expert appears in at least 8 routes. Full route-exec parity passes in every mode. Results: six-fixture spread dense **2.085s** vs hybrid **1.131s**; prefill4 all-layer route-exec dense **18.573s** vs hybrid **9.405s** (**1.98x**); prefill16 all-layer route-exec dense **42.078s** vs hybrid **29.559s** (**1.42x**). Online `forward-one` top-k1 with the C++ daemon keeps next token `1655` and improves dense **1.450s** to hybrid **0.895s** (**1.62x** total forward, **2.38x** routed compute). Follow-up online fanout artifact `results/20260708-cpp-route-q4-online-topk-summary.json` confirms the same cold-route win at less-toy fanout: top-k5 dense **6.678s** vs hybrid **4.082s** (**1.64x**, next token `2`), top-k8 dense **9.517s** vs hybrid **5.736s** (**1.66x**, next token `1655`), all with swap delta **0.0GiB**. Verdict: first runtime-shaped C++/Capstan Hy3 compute cut that is a real speed win; do not promote hybrid as the global default until multi-token `generate-cache` / hot-cache decode is measured.
