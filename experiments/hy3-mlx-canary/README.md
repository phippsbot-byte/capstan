...
- Packed layer-major sidecar is now the default Python layout and removed the worst Python-GC/read-call bottleneck; bounded packed-read coalescing (`32MiB` cap, `2x` overread ceiling) cuts top5/slot16 decode8 wall from `76.6s` to `59.7s` in the current smoke.
...
- First Capstan/C++ substrate exists under `cpp/`: split reusable expert-bank/q4/routed-MLP modules + compact index + contiguous expert-span `pread` benchmark + native slot-bank cache simulation + real router trace replay + routed q4 parity. Full top-8 all-layer expert payload (6.249GiB / 632 spans) reads in ~3.2s warm-cache on the Studio; a real top5/slot16 prefill+decode trace replays 20.853GiB with 1,051 hits / 2,109 misses / 845 evictions. All 79 MoE layers pass native routed parity. Layer-major fixture replay now dedups repeated prompt experts: the 4-token all-layer fixture drops from 1,580 naïve expert reads / 15.6226GiB to 1,408 unique reads / 13.9219GiB with parity intact. Apple Accelerate qlinear plus expert-major dense reuse cuts prefill16 layer-major wall from 899.48s to 239.45s; `--route-exec` exposes the same routed-MoE math as fixture-backed prompt-slice execution, with explicit routed-only scope and stricter fixture-list validation. A persistent C++ routed-MoE daemon and opt-in Python hook now work, including dense expert caching, but online top-k1 CPU dense path is slower than MLX. A simple custom Metal q4 matvec spike is only parity/noisy vs `gather_qmm`; next speed work needs fused/route-batched Metal, not another one-projection matvec clone.
