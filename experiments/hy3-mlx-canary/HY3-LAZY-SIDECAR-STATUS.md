...
C++ direct q4 dot spike: artifact `spikes/004-cpp-q4-direct-dot/results/20260708-q4-direct-dot.json`. This tested the algebraic cold-route path `dot(x, q * scale + bias) = scale * sum(q*x) + bias * sum(x)` with a NEON implementation that avoids dense FP32 materialization. On six real seq4/seq16 fixtures (layers 1/40/79), direct q4 was **2.6212x** median / **2.9841x** mean faster than current dense-dequant + Accelerate when dequantization is counted, with `max_rel_to_dense < 5e-7`. But once dense weights are already materialized and cached, direct q4 is only **0.4062x** median vs dense compute. Verdict: validated for cold/prefill-shaped routes, not hot-cache decode.

Hybrid q4 runtime cut: artifact `cpp/results/20260708-hybrid-q4-summary.json`. C++ routed-MoE replay and the persistent daemon now expose `dense|direct|hybrid` q4 modes (`--q4-mode`, plus Python `HY3_CPP_ROUTE_Q4_MODE`). Default remains `dense`; `hybrid` uses direct q4 for cold/low-reuse experts and dense+Accelerate when an expert appears in at least 8 routes, and the daemon now also routes already-cached dense experts through dense. Full route-exec parity passes in every mode. Results: six-fixture spread dense **2.085s** vs hybrid **1.131s**; prefill4 all-layer route-exec dense **18.573s** vs hybrid **9.405s** (**1.98x**); prefill16 all-layer route-exec dense **42.078s** vs hybrid **29.559s** (**1.42x**). Online `forward-one` top-k1/top-k5/top-k8 canaries show hybrid/direct remain **1.6x-ish** faster with **0.0GiB** swap delta. Multi-token generate-cache artifact `results/20260708-cpp-route-q4-generate-cache-4tok-cache16-combined-summary.json` confirms the speed win survives decode-shaped execution with `HY3_CPP_ROUTE_DENSE_CACHE_GIB=16`: top-k5 dense **84.915s** vs hybrid **52.600s** (**1.61x**), top-k8 dense **120.941s** vs hybrid **82.670s** (**1.46x**), all with **0.0GiB** swap delta. Forced-token logit-delta artifact `results/20260708-cpp-route-q4-forced-logits-delta-summary.json` shows the token drift is mostly knife-edge top-2 swaps rather than semantic collapse: top-k5 direct drift is step1 dense top1 ranked #2 under direct, top-k5 hybrid drift is step0 dense top1 ranked #2 under hybrid, and top-k8 hybrid drift is step0 dense top1 ranked #2 under hybrid; top-k8 direct matched dense top1 for all four forced steps. Verdict: real runtime speed lane, but still not default-safe; keep default dense, treat direct top-k8 as the cleanest speed candidate, and only promote direct/hybrid after logit tolerance is accepted or numerics are tightened enough to avoid argmax drift.

Packed direct-cache cut: the C++ daemon now has an opt-in byte-bounded persistent packed `ExpertBank` LRU (`--packed-cache-gib`; Python `HY3_CPP_ROUTE_PACKED_CACHE_GIB`) for direct and direct-side hybrid routes. `ExpertBank` is move-only and explicitly rebinds all `TensorSlice` pointers after moves; CTest covers ownership/LRU/bounds. Dense plus packed budgets are jointly capped at **16GiB**, and packed cache defaults off without retaining a scratch bank. A repeated layer1/top5 request was byte-identical with first request **5 reads / 50.6MiB** and second request **0 reads / 5 hits**; raw proof is `results/20260709-cpp-route-packed-cache-repeat-smoke.json`. On the existing top-k8 direct four-token shape, 16GiB packed cache preserved `[185, 120029, 72520, 423]`, cut step sum **91.766s → 82.298s** (**10.3%**), cut median decode **3.944s → 2.994s** (**24.1%**), reduced reads **82.048GiB → 75.493GiB** (**8.0%**), and kept swap delta **0.0GiB**. A layer-sharded LRU produced no material hit/read improvement and was discarded. Artifact: `results/20260709-cpp-route-packed-cache-summary.json`. Verdict: ship the simple global packed LRU as an opt-in decode optimization; do not claim the initial 15% aggregate read target, because prefill route overlap is the limiting factor.

Packed-client hardening follow-up: independent review found the Python singleton could interleave daemon frames across threads or reuse inherited pipes after `fork()`, and that six-significant-digit cache formatting could turn an accepted 16GiB boundary into a daemon rejection. The client now serializes the full exchange, locks singleton create/reset/stats, tracks owner PID, resets its lock after fork, and spawns a child-owned daemon on next access. Cache budgets use `.17g`; C++ numeric parsing requires full-string consumption. A real sidecar stress passed **16 concurrent singleton lookups**, **4 concurrent exchanges**, fork-child daemon replacement with parent survival, exact `15.99999 + 0.00001GiB` acceptance, and `1junk` rejection. Artifact: `results/20260709-cpp-route-client-hardening-smoke.json`.
