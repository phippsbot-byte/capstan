...
Metal compute spikes: the simple one-projection MLX custom Metal q4 matvec is feasible but only parity/noisy against `gather_qmm`. The follow-up fused up+gate spike lives at `spikes/002-metal-fused-up-gate/` with summary `spikes/002-metal-fused-up-gate/summary.json`. Stable six-sample pass: fused up+gate+swiglu beats `gather_qmm(up/gate)+swiglu` consistently (**1.0819x** median speedup, **1.0343x** minimum), so the fused q4 Metal direction is real. But the full expert proxy — fused up+gate followed by normal gather-qmm down — is only **1.0536x** median / **1.0736x** mean and one sample regresses (**0.9832x**). Verdict: do not wire up+gate-only Metal into runtime. Next serious kernel needs to fuse more of the route (`down` and/or route weighting) and avoid dense FP32 materialization plus tiny-M CPU/BLAS overhead.
